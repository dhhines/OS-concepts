...
{
    // pid_t variable for process ID of the root parent process
    //pid_t pid1 = 0, pid2 = 0, pid3 = 0, pid4 = 0;
    ...
    //array of file names to open and search
    //char *files[argc - 2];
    //for (int i = 0; i < argc - 2; i++)
    //    (files[i], argv[i + 2]);


    //search and print in the first text file which is 3rd argument
    //searchAndPrint(argv[1], argv[2]);
    //searchAndPrint(word, &files[0]);

/*
    // run only for the child process
    if (argc >= 4){
        // get the pid returned from fork()
        pid = fork();

        // select for the child process
        if (pid == 0){
            searchAndPrint(argv[1], argv[3]);

            // verify there are additional text files to search
            if (argc >= 5){
                ...
                if (pid2 == 0){
                    ...
                    if (argc >= 6){
                        ...
                        if (pid3 == 0){
                            ...
                            searchAndPrint(argv[1], argv[5]);
                            if (argc >= 7){
                                pid4 = fork();

                                if (pid4 == 0){
                                    searchAndPrint(argv[1], argv[6]);
                                    exit(0);
                                }
